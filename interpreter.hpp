...
namespace example
{
  ...
  class NotAFunction //: std::exception
  {
	  std::string name_;
  public:
	  explicit NotAFunction(const std::string& function_name) : name_(function_name) {}
	  const char* what() 
	  { 
		  name_ = "not such function to call: " + name_;
		  return name_.c_str();
	  }
  };
  ...
  struct interpreter::invoker
  {
    ...
    static inline
    void apply_object(Function func, TheClass* theclass, token_parser & parser, Args const & args)
    {
        typedef typename mpl::next<From>::type next_iter_type;
        interpreter::invoker<Function, next_iter_type, To>::apply( func, parser, fusion::push_back(args, theclass) );        
     } 
  ...
  interpreter::register_function(std::string const & name, Function f)
  {
    ...
    template<typename Function, typename TheClass>
    typename boost::enable_if< ft::is_member_function_pointer<Function> >::type
    interpreter::register_function(std::string const& name, Function f, TheClass* object)
    {   
        // instantiate and store the invoker by name
        this->map_invokers[name] = boost::bind(& invoker<Function>::template  apply_object<fusion::nil,TheClass>, f, object, _1,fusion::nil());
    }
